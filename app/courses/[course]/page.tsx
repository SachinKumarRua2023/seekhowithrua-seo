...
export async function generateMetadata({ params }: { params: { course: string } }): Promise<Metadata> {
  ...
  return {
    title: `${course.title} Course Curriculum | SeekhowithRua`,
    description: course.description,
    keywords: course.keywords,
    alternates: { canonical: `https://seekhowithrua.com/courses/${params.course}` },
    openGraph: {
      title: `${course.title} | SeekhowithRua`,
      description: course.description,
      url: `https://seekhowithrua.com/courses/${params.course}`,
      type: "website",
    },
  };
}

export default function CourseOverviewPage({ params }: { params: { course: string } }) {
  const course = ALL_COURSES[params.course];
  if (!course) notFound();
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#04040f", color: "#e2e8f0", padding: "80px 40px 60px", fontFamily: "'Exo 2', sans-serif" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <Link href={`/${course.slug}`} style={{ color: course.color, textDecoration: "none", fontSize: 13, fontFamily: "monospace" }}>
            ← Back to {course.title} Course
          </Link>

          <h1 style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "clamp(28px,4vw,44px)", fontWeight: 700, color: "#fff", margin: "20px 0 12px" }}>
            {course.icon} {course.title} — Full Curriculum
          </h1>
          ...
          {course.modules.map(mod => (
            <div key={mod.id} style={{ marginBottom: 32, padding: "24px", border: "1px solid rgba(255,255,255,0.07)", borderLeft: `4px solid ${mod.color}`, borderRadius: 14, background: "rgba(10,10,30,0.6)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px,1fr))", gap: 10 }}>
                {mod.chapters.map(ch => (
                  <Link key={ch.id} href={`/courses/${params.course}/${ch.id}`}
          ...
          <div style={{ marginTop: 40, padding: "28px", border: `1px solid ${course.color}40`, borderRadius: 14, background: `${course.color}08`, textAlign: "center" }}>
            ...
            <a href="https://app.seekhowithrua.com"
              style={{ display: "inline-block", padding: "12px 28px", borderRadius: 8, background: `linear-gradient(135deg,${course.color},#7c3aed)`, color: "#fff", fontFamily: "'Rajdhani', sans-serif", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
              Start Learning Free →
            </a>
          </div>
        </div>
